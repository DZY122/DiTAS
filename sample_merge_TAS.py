...
"""
Sample new images from a quantized DiT.
"""
import torch
...
def main(args):
    ...
    Run sampling.
    """
    # global global_act_bit, global_weight_bit
    ...
    replace_module_sample(model, device)

    set_inference(model)

    ckpt_path = args.path
    state_dict = torch.load(ckpt_path, map_location=lambda storage, loc: storage)["model"]
    model.load_state_dict(state_dict)
    ...
    sample_folder_dir = f"{args.sample_dir}/{folder_name}/A{global_act_bit}W{global_weight_bit}layer_merge_fianl_test"
    if rank == 0:
